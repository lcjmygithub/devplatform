  ...
  <div>
    <div class="api-details">
      <div class="title_h1">
        ...
        <div class="info">通过接口可以获取查询剩余并发列表</div>
      </div>
      <div class="get-url">
        <div class="basic">
          <h1 style="font-size: 20px;">请求地址</h1>
          <div class="info">
             {{this.$store.state.baseURL+'open/getSurplusOccurs?token='+token}}
...
<script>
const dataJson = require('../../../../json/companyAPI/companyAPI_2.json')
export default {
  data() {
    return {
        token:'',
       ...
       requesList:[
        ...
        {title:'正确示例',active:false}
         ...
         tableData1:[
          ...
          return_required: '否',
          return_types:'Object',
          return_introstactor:'并发数信息',
          //子项
           children: [
             {
                return_refsName: 'maxOccurs',
                return_required: '否',
                return_types:'Number',
                return_introstactor:'最大并发数'
             },
              {
                return_refsName: 'surplusOccurs',
                return_required: '否',
                return_types:'Number',
                return_introstactor:'剩余并发',
             },
             {
                return_refsName: 'useOccurs',
                return_required: '否',
                return_types:'Number',
                return_introstactor:'使用并发',
             },
           ]
          }
        ],
  ...
  methods: {
    ...
    //在线调式
      ...
      resetForm(formName) {
       ...
       //复制 文本
       doCopy(e) {
             this.$copyText(this.$store.state.baseURL+'open/getSurplusOccurs?token='+this.token).then((e) => {
               // success
                 this.$message({
                showClose: false,
                center:true,
