  ...
  <div>
    <div class="api-details">
      <div class="title_h1">
        ...
        <div class="info">通过接口可以添加单个黑名单</div>
      </div>
      <div class="get-url">
        <div class="basic">
          <h1 style="font-size: 20px;">请求地址</h1>
          <div class="info">
             {{this.$store.state.baseURL+'open/saveOrUpdateZtBlackList?token='+this.token}}
...
<script>
const dataJson = require('../../../../json/companyAPI/companyAPI_3.json')
export default {
  data() {
    return {
         token:'',
       ...
       requesList:[
        ...
        {title:'正确示例',active:false}
         ...
         tableData1:[
          ...
          return_required: '否',
          return_types:'String',
          return_introstactor:'添加黑名单操作的提示内容',
          }
        ],
  ...
  methods: {
    ...
    //在线调式
      ...
      resetForm(formName) {
       ...
       //复制 文本
       doCopy(e) {
             this.$copyText(this.$store.state.baseURL+'open/saveOrUpdateZtBlackList?token='+this.token).then((e) => {
               // success
                 this.$message({
                showClose: false,
                center:true,
